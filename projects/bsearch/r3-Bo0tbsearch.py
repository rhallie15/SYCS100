...
# Above is my list of pretty nifty words.
def bsearch(list, b): # Function takes the parameters list and 'b' (the object to find)
	...
	while (not search is False): # Keeps looping until the list is empty
		...
		if list[search[i]] < b or b < list[search[0]]: # if the thing you're looking for is bigger or smaller than the list that the function is looking at, then its probably not there. So it returns -1 and quits
			return -1
			...
			return True, search[i]
		else: # if none of the above apply, it'll cut the list in the following way
			i /= 2 #Cuts i in half
			if (i == 0): #if i is zero, then we prolly only have one more place to search. And if that ain't it, then we're s-o-l.
				if list[search[i]] == b:
				...
				if b > list[search[i]]: # if the list search at position i is less than b, then the search is cut to being the remaining numbers to the right of i
					...
					search = search[i+1:]
				else:# if the list search at position i is greater than b, then the search is cut to being the remaining numbers to the left of i
				
					search = search[:i+1]
	else: #if it gets through the entire list and didn't find anything, then you can have a -1
		return -1

print bsearch(list,'aardvark')
print bsearch(list,'dog')
print bsearch(list,'guts')
